...
export class GameLogic {
	...
	public executeAction(gameState: GameState, command: Command): ActionResult {
		...
		switch (command.action) {
			...
			default:
				return {
					success: false,
					gameResult: GameResult.LOSE_BY_INVALID_COMMAND,
				};
		}
	}
	...
	private executeWalk(
		...
		const opponent =
			player.type === PlayerType.COOL
				? gameState.hotPlayer
				: gameState.coolPlayer;
		if (
			opponent.position.x === newPosition.x &&
			opponent.position.y === newPosition.y
		) {
			return { success: false, gameResult: GameResult.LOSE_BY_WALL_HIT };
		}
	...
	private executePut(
		...
		const targetPosition = this.getPositionInDirection(
			player.position,
			direction,
		);
		...
		const opponent =
			player.type === PlayerType.COOL
				...
				: gameState.coolPlayer;
		if (
			opponent.position.x === targetPosition.x &&
			opponent.position.y === targetPosition.y
		) {
			gameState.isGameOver = true;
			gameState.winner = player.type;
				...
				if (this.isOutOfBounds(gameState, checkPosition)) {
					...
					const opponent =
						player.type === PlayerType.COOL
							...
							: gameState.coolPlayer;
					if (
						(player.position.x === checkPosition.x &&
							player.position.y === checkPosition.y) ||
						(opponent.position.x === checkPosition.x &&
							opponent.position.y === checkPosition.y)
					) {
						lookResult.push(CellType.PLAYER);
					} else {
						lookResult.push(
							gameState.map.cells[checkPosition.y][checkPosition.x],
						);
					}
				}
			}
	...
	private executeSearch(
		...
		for (let i = 1; i <= 9; i++) {
			const checkPosition = this.getPositionInDirection(
				player.position,
				direction,
				i,
			);
	...
	public checkGameEnd(gameState: GameState): void {
		if (gameState.currentTurn >= gameState.map.maxTurns) {
			...
			if (gameState.coolPlayer.itemCount > gameState.hotPlayer.itemCount) {
				...
				gameState.hotPlayer.itemCount > gameState.coolPlayer.itemCount
			) {
