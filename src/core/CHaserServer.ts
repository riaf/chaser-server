...
export class CHaserServer {
	...
	// Expose for testing
	public get testGameLogic() {
		return this.gameLogic;
	}
	public set testGameState(state: GameState | undefined) {
	...
	public async processTurn(): Promise<void> {
		...
		const currentPlayerId =
			this.gameState.currentPlayer === PlayerType.COOL ? "cool" : "hot";

		// Wait for turn request
		const turnRequest =
			await this.connectionHandler.receiveMessage(currentPlayerId);
		if (turnRequest !== "gr") {
		...
		const surroundingInfo =
			this.surroundingInfoGenerator.generateSurroundingInfo(
				this.gameState,
		...
		const updatedSurroundingInfo =
			this.surroundingInfoGenerator.generateSurroundingInfo(
				this.gameState,
				this.gameState.currentPlayer,
			);
		await this.connectionHandler.sendMessage(
			currentPlayerId,
			updatedSurroundingInfo,
		);
		...
		if (result.gameResult) {
			...
			if (result.gameResult.includes("win")) {
				...
				this.gameState.winner =
					this.gameState.currentPlayer === PlayerType.COOL
						? PlayerType.HOT
						: PlayerType.COOL;
			}
		}
