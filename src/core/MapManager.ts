import { GameMap, CellType, Position } from '../types'
  ...
  public loadMapFromString(mapString: string): GameMap {
    ...
    for (const line of lines) {
      if (line.startsWith('N:')) {
        ...
        const [h, w] = line.substring(2).split(',').map(n => Number.parseInt(n, 10))
        height = h
        width = w
      } else if (line.startsWith('D:')) {
        const row = line.substring(2).split(',').map(n => Number.parseInt(n, 10) as CellType)
        cells.push(row)
      } else if (line.startsWith('C:')) {
        const [y, x] = line.substring(2).split(',').map(n => Number.parseInt(n, 10))
    ...
    if (!this.validatePointSymmetry(cells)) {
      throw new Error('アイテムとブロックは中心点対称に配置されている必要があります')
    }
