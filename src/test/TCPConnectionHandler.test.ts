...
describe("TCPConnectionHandler", () => {
	...
	describe("constructor", () => {
		...
		it("should create handler with custom ports", () => {
			const customHandler = new TCPConnectionHandler({
				coolPort: 30000,
				hotPort: 35000,
			});
	...
	describe("sendMessage", () => {
		it("should reject when player not connected", async () => {
			await expect(handler.sendMessage("cool", "test message")).rejects.toThrow(
				"Player cool is not connected",
			);
		});
	});
	...
	describe("receiveMessage", () => {
		it("should reject when player not connected", async () => {
			await expect(handler.receiveMessage("cool")).rejects.toThrow(
				"Player cool is not connected",
			);
		});
	});
	...
	describe("waitForConnections", () => {
		it("should timeout if no connections are made", async () => {
			...
			await expect(
				Promise.race([handler.waitForConnections(), timeout]),
